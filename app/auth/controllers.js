...
const verifyCode = async(req, res) =>{
    ...
    if(!authCode){
        ...
        console.log(authCode.valid_till);
...
const logIn = async(req, res) =>{
    if(!req.body.emai || req.body.emai.length === 0 || 
        !req.body.password || req.body.password.length === 0){
            ...
            if(!user) return res.status(401).send({message: "User with this email is not exist"})
            ...
            if(isMatch){
                ...
                res.status(401).send({message: "Password is incorect"})
            }
        }
}
